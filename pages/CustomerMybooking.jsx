...
import { NEARBY_WORKERS } from "../data/mockauth";
...
const getBookingStatus = (booking) => {
  ...
  if (
    ...
    return status.includes("cancel") ? "Cancelled" : "Completed";
  }
...
export default function CustomerMyBookings() {
  ...
  const getWorkerForBooking = (booking) => {
    ...
    const worker = NEARBY_WORKERS.find((item) => {
      const normalized = normalizeWorker(item, 0);

      return (
        (bookingWorkerId &&
          String(normalized.id) ===
            String(bookingWorkerId)) ||
        (bookingWorkerName &&
          normalized.name.toLowerCase() ===
            String(bookingWorkerName).toLowerCase())
      );
    });

    if (worker) {
      return normalizeWorker(worker, 0);
    }
