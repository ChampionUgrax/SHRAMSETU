...
// around the worker's avatar while we poll for their Accept/Reject, with a
// visible countdown to when we'll try the next-nearest worker instead.
function WaitingForApproval({ worker, elapsed, attempt, maxAttempts, onCancel }) {
  const initials = worker.name.split(' ').map((n) => n[0]).join('').slice(0, 2)
  const secondsLeft = Math.max(0, TIMEOUT_SECONDS - elapsed)
  return (
    <div className="flex flex-col items-center text-center py-6">
      ...
      {attempt > 1 && (
        <p className="text-xs text-saffron-600 font-semibold mt-2">Previous worker didn't respond — trying the next nearest one</p>
      )}
      <p className="text-xs text-navy-300 mt-3">Attempt {attempt} of {maxAttempts} &middot; trying another worker in {secondsLeft}s</p>
...
function NoWorkersScreen({ onRetry, onBrowse }) {
  return (
    <div className="flex flex-col items-center text-center py-6">
      <div className="w-16 h-16 rounded-full bg-navy-50 text-navy-400 flex items-center justify-center mb-4">
        <Users size={28} />
      </div>
      <p className="font-display font-bold text-lg text-navy-700">No workers responded right now</p>
      <p className="text-sm text-navy-400 mt-1 max-w-xs">We tried the nearest available workers but none accepted in time. You can try again or browse the marketplace.</p>
      <div className="flex gap-3 mt-6">
        <Button variant="outline" onClick={onRetry}><RefreshCw size={14} /> Try Again</Button>
        <Button variant="primary" onClick={onBrowse}>Browse Workers</Button>
      </div>
    </div>
  )
}
...
export default function Booking() {
  ...
  const initialWorker = WORKERS.find((w) => w.id === workerId) || WORKERS[0]
  const isEmergency = params.get('emergency') === '1'

  // The worker actively being requested — changes as we auto-reassign.
  const [activeWorker, setActiveWorker] = useState(initialWorker)
  ...
  // Approval flow state: 'form' | 'waiting' | 'declined' | 'no-workers' | 'confirmed'
  const [phase, setPhase] = useState('form')
  const [jobId, setJobId] = useState(null)
  const [elapsed, setElapsed] = useState(0)
  const [attempt, setAttempt] = useState(1)
  const [triedWorkerIds, setTriedWorkerIds] = useState([])
  const [reassigning, setReassigning] = useState(false)

  const availableServices = useMemo(
    () => SERVICES_MENU.filter((s) => s.category === initialWorker.category),
    [initialWorker]
  )
  ...
  useEffect(() => {
    let cancelled = false
    getAvailability(activeWorker.id).then((data) => {
      if (cancelled) return
  ...
  }, [initialWorker.id, days])

  const date = days[selectedDayIdx]
  const amount = (service?.basePrice || activeWorker.price) + (isEmergency ? 100 : 0)
  ...
  const requestSpecificWorker = async (targetWorker) => {
    const job = await createJobRequest({
      ...
      time,
      distance: targetWorker.distance,
      earnings: amount,
    })
    setJobId(job.id)
    setPhase('waiting')
  ...
  const handleRequestWorker = async () => {
    setSubmitting(true)
    try {
      await requestSpecificWorker(activeWorker)
    } finally {
      setSubmitting(false)
    }
  }

  const finalizeBooking = async (bookedWorker) => {
    const booking = await createBooking({
      workerId: bookedWorker.id,
  ...
  // Auto-reassign to the next-nearest worker in the same category
  const attemptNextWorker = async () => {
    if (jobId) await updateJobStatus(jobId, 'Cancelled')
    const newTried = [...triedWorkerIds, activeWorker.id]

    const candidates = WORKERS
      .filter((w) => w.category === activeWorker.category && !newTried.includes(w.id))
      .sort((a, b) => a.distance - b.distance)

    if (attempt >= MAX_ATTEMPTS || candidates.length === 0) {
      setTriedWorkerIds(newTried)
      setPhase('no-workers')
      return
    }
    ...
    setElapsed(0)
    setJobId(null)
    await requestSpecificWorker(next)
  }
  ...
  // Timeout watcher: once elapsed hits the limit, auto-reassign
  useEffect(() => {
    if (phase !== 'waiting' || reassigning) return
    if (elapsed >= TIMEOUT_SECONDS) {
      setReassigning(true)
      attemptNextWorker().finally(() => setReassigning(false))
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [elapsed, phase])
    ...
    return (
      <div className="container-app py-14 flex justify-center">
        <div className="card p-8 max-w-lg w-full text-center animate-fade-up">
          ...
          <p className="text-navy-400 text-sm mt-1">{confirmedBooking.workerName} accepted your request.</p>
  ...
  return (
    <div className="container-app py-8">
      ...
      <p className="text-navy-400 text-sm mt-1">with {activeWorker.name} &middot; {activeWorker.skill}</p>
      ...
      <div className="card p-6 max-w-2xl mt-6">
        ...
        {phase === 'declined' && (
          <DeclinedScreen worker={activeWorker} onRetry={handleRetryFromScratch} onFindAnother={() => navigate('/services')} />
        )}

        {phase === 'no-workers' && (
          <NoWorkersScreen onRetry={handleRetryFromScratch} onBrowse={() => navigate('/services')} />
        )}
          ...
          <>
            ...
            {step === 1 && (
              availability ? (
                ...
                <div className="flex items-center gap-2 text-navy-400 text-sm py-6"><Loader2 size={16} className="animate-spin" /> Checking {activeWorker.name.split(' ')[0]}'s availability...</div>
              )
            )}
            ...
            {step === 4 && (
              <div>
                ...
                <div className="bg-navy-50 rounded-xl p-5 space-y-2 text-sm">
                  <Row label="Worker" value={activeWorker.name} />
                ...
                <p className="text-xs text-navy-300 mt-3">
                  Your request goes to {activeWorker.name} first — if they don't respond within {TIMEOUT_SECONDS}s, we'll automatically try the next nearest worker (up to {MAX_ATTEMPTS} attempts).
                </p>
              </div>
            )}
